  ...
  return (
    <div className="flex flex-col flex-1 bg-gray-900">
      <div className="flex-1 overflow-y-auto p-4">
        {messages.length === 0 && (
          <p className="text-center text-gray-500 mt-10">Upload a document below, then ask a question to get started.</p>
        )}
        {messages.map((m, i) => (
      ...
      <div className="p-4 border-t border-gray-800 flex gap-2 items-center">
        ...
        <input
          ...
          disabled={loading}
          className="flex-1 bg-gray-800 text-white placeholder-gray-500 border border-gray-700 rounded-lg px-3 py-2 disabled:opacity-50"
        />
        <button
          onClick={handleSend}
          disabled={loading}
          className={`px-4 py-2 rounded-lg text-white ${loading ? "bg-blue-900 cursor-not-allowed" : "bg-blue-600"}`}
        >
          Send
        </button>
      </div>
